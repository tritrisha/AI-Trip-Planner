...
var VelocityTracker = /*#__PURE__*/ function() {
	...
	VelocityTracker.track = function track(targets, properties, types) {
		_coreInitted$1 || _initCore$1();
		var result = [], targs = _toArray$1(targets), a = properties.split(","), t = (types || "").split(","), i = targs.length, tracker, j;
		while (i--) {
...
var _emptyFunc = function _emptyFunc(p) {
	return p;
};
var _bigNum = 1e10;
...
var _getClosest = function _getClosest(n, values, max, min, radius) {
	...
	if (_isObject(n)) {
		...
		if ((radius || _bigNum) < _bigNum && radius < Math.sqrt(absDif)) return n;
	} else while (i--) {
		val = values[i];
		dif = val - n;
		if (dif < 0) dif = -dif;
...
var _parseEnd = function _parseEnd(curProp, end, max, min, name, radius, velocity) {
	...
	} else adjustedEnd = _isFunction(endVar) ? endVar(end, velocity) : _isArray(endVar) ? _getClosest(end, endVar, max, min, radius) : parseFloat(endVar);
...
//#endregion
export { InertiaPlugin, InertiaPlugin as default, VelocityTracker };
